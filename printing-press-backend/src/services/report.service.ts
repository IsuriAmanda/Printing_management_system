...
// ── QUOTATION REPORT ──────────────────────────────────────────
export const getQuotationReport = async (
  from?: string,
  to?: string
) => {
...
`, params);
  ...
  // Machine utilization
  const [machineStats] = await pool.query<RowDataPacket[]>(`
    SELECT
      m.machine_id,
      m.name,
...
export const getDashboardStats = async () => {
  const [quotationStats] = await pool.query<RowDataPacket[]>(`
    SELECT
      COUNT(*)                                              AS total,
      SUM(CASE WHEN status = 'CONFIRMED' THEN 1 ELSE 0 END) AS confirmed,
      SUM(CASE WHEN status = 'CANCELLED' THEN 1 ELSE 0 END) AS cancelled,
 ...
 const [jobStats] = await pool.query<RowDataPacket[]>(`
  SELECT
    COUNT(*)                                                                 AS total,
    SUM(CASE WHEN status = 'COMPLETED' THEN 1 ELSE 0 END)    AS completed,
    SUM(CASE WHEN status != 'COMPLETED' THEN 1 ELSE 0 END)   AS in_progress,
...
`);
  ...
  const [recentQuotations] = await pool.query<RowDataPacket[]>(`
    SELECT
      q.id,
      q.quotation_number,
      c.name   AS customer_name,
