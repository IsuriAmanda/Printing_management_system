...
const fetchQuotationForPdf = async (id: number) => {
...
export const generateQuotationPdf = async (id: number): Promise<Buffer> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(8).fillColor(WHITE).font('Helvetica')
       .text(
         'Harshana Printers  |  222/7, 1st Lane, Kalapaluwawa, Rajagiriya  |  Tel: 011-XXXXXXX',
         margin, FOOTER_Y + 12,
    ...
    doc.fontSize(9).fillColor(GRAY).font('Helvetica-Bold')
       .text('TERMS & CONDITIONS', margin, termsY + 8,
         { lineBreak: false });
    ...
    terms.forEach((line, i) => {
      doc.text(line, margin, termsY + 22 + i * 13,
        { lineBreak: false, width: cW });
    });
    ...
    doc.fontSize(11).fillColor(WHITE).font('Helvetica')
       .text(`No: ${qNum}`, pageW - 230, 132,
         { width: 178, align: 'right', lineBreak: false });
    ...
    doc.fontSize(11).fillColor(BLACK).font('Helvetica-Bold')
       .text(data.customer_name || '', margin + 10, infoY + 24,
         { lineBreak: false });

    doc.fontSize(9).fillColor(BLACK).font('Helvetica');
    if (data.customer_phone) {
      doc.text(`Tel: ${data.customer_phone}`, margin + 10, infoY + 42,
        { lineBreak: false });
    }
    if (data.customer_email) {
      doc.text(`Email: ${data.customer_email}`, margin + 10, infoY + 56,
        { lineBreak: false });
    }
    ...
    const colorDesc = (v: number) =>
      v === 4 ? '4 Color' : v === 1 ? '1 Color' : null;

    const descParts: string[] = [data.job_name || data.description || ''];
    if (data.job_type)           descParts.push(`Type: ${data.job_type}`);
    if (data.pages)              descParts.push(`Pages: ${data.pages}`);
    const fc = colorDesc(data.front_color_pages);
    const bc = colorDesc(data.back_color_pages);
    if (fc) descParts.push(`Pages Front: ${fc}`);
    if (bc) descParts.push(`Pages Back: ${bc}`);
    if (data.binding_name)       descParts.push(`Binding: ${data.binding_name}`);
    if (data.lamination_name)    descParts.push(`Lamination: ${data.lamination_name}`);

    doc.fontSize(9).fillColor(BLACK).font('Helvetica')
       .text(descParts.join('\n'), col.desc.x + 5, rowY + 8,
         { width: col.desc.w - 10, lineGap: 2, lineBreak: true });

    doc.text(`${data.quantity || 0}`,
       col.qty.x + 5, rowY + 8,
       { width: col.qty.w - 10, lineBreak: false });

    doc.text(`${Number(data.width || 0).toFixed(2)} x ${Number(data.height || 0).toFixed(2)} in`,
       col.size.x + 5, rowY + 8,
       { width: col.size.w - 10, lineBreak: false });

    doc.text(Number(data.item_total || 0).toFixed(2),
       col.amt.x + 5, rowY + 8,
       { width: col.amt.w - 10, align: 'right', lineBreak: false });
    ...
    const money = (value: number) => `LKR ${value.toFixed(2)}`;
    ...
    breakdownRows.forEach(([label, value], index) => {
      ...
      if (index % 2 === 0) doc.rect(rightX, y, rightW, 27).fill('#fafafa');
      doc.fontSize(9).fillColor(BLACK).font(index === 0 ? 'Helvetica-Bold' : 'Helvetica')
         ...
         .text(value, rightX + rightW * 0.58, y + 9,
           { width: rightW * 0.37, align: 'right', lineBreak: false });
    ...
    doc.fontSize(11).fillColor(WHITE).font('Helvetica-Bold')
       .text('TOTAL', rightX + 12, totalY + 14,
         { width: rightW * 0.42, lineBreak: false })
       .text(money(totalCost), rightX + rightW * 0.42, totalY + 14,
         { width: rightW * 0.53, align: 'right', lineBreak: false });
