...
exports.finishPrintingForJob = exports.startPrintingOnMachine = exports.markAttendance = exports.updateMachineStatus = exports.getOperatorMachineAssignments = exports.getMachineStatusBoard = exports.getDailyAssignments = exports.setDailyAssignment = void 0;
...
const normalizeAssignmentDate = (date) => {
...
const setDailyAssignment = async (machineId, userId, date) => {
    ...
    const [attendanceRows] = await db_1.default.query(`SELECT COALESCE(oa.is_present, 1) AS is_present
     FROM users u
     LEFT JOIN operator_attendance oa ON oa.user_id = u.id AND oa.attendance_date = ?
     WHERE u.id = ? AND u.role_id = 3
     LIMIT 1`, [assignmentDate, userId]);
    ...
    await db_1.default.query(`INSERT INTO machine_daily_assignment (machine_id, user_id, assignment_date) VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE user_id = VALUES(user_id)`, [machineId, userId, assignmentDate]);
    const [details] = await db_1.default.query(`
  ...
  `, [userId, machineId]);
    ...
    if (detail) {
        await (0, notification_service_1.createNotification)(userId, `You are assigned to machine ${detail.machine_name} for ${assignmentDate}`, 'ASSIGNMENT');
        if (detail.email) {
            /* TEMPORARILY DISABLED (requires internet/SMTP)
...
exports.setDailyAssignment = setDailyAssignment;
const getDailyAssignments = async (date) => {
...
exports.getDailyAssignments = getDailyAssignments;
const getMachineStatusBoard = async (date) => {
    const [rows] = await db_1.default.query(`
    SELECT
      m.machine_id,
...
exports.getMachineStatusBoard = getMachineStatusBoard;
const getOperatorMachineAssignments = async (userId, date) => {
    const [rows] = await db_1.default.query(`
...
exports.getOperatorMachineAssignments = getOperatorMachineAssignments;
const updateMachineStatus = async (machineId, isActive) => {
    const [result] = await db_1.default.query(`UPDATE machine SET is_active = ? WHERE machine_id = ?`, [isActive ? 1 : 0, machineId]);
    if (result.affectedRows === 0) {
        throw new Error('Machine not found');
    }
    if (!isActive) {
        await db_1.default.query(`DELETE FROM machine_daily_assignment WHERE machine_id = ? AND assignment_date >= CURDATE()`, [machineId]);
    }
    const [rows] = await db_1.default.query(`SELECT machine_id, name AS machine_name, is_active FROM machine WHERE machine_id = ?`, [machineId]);
    return rows[0];
};
exports.updateMachineStatus = updateMachineStatus;
const getAssignedOperatorForMachineToday = async (machineId, date) => {
    const [rows] = await db_1.default.query(`SELECT user_id FROM machine_daily_assignment WHERE machine_id = ? AND assignment_date = ?`, [machineId, date]);
    return rows.length ? rows[0].user_id : null;
};
const markAttendance = async (userId, date, isPresent) => {
    await db_1.default.query(`INSERT INTO operator_attendance (user_id, attendance_date, is_present) VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE is_present = VALUES(is_present)`, [userId, date, isPresent ? 1 : 0]);
};
exports.markAttendance = markAttendance;
// ── Is this machine currently locked by an in-progress job? ──
const getActiveAssignmentForMachine = async (machineId) => {
    const [rows] = await db_1.default.query(`SELECT ja.*
     FROM job_assignment ja
     JOIN job_ticket jt ON jt.job_id = ja.job_id
...
const startPrintingOnMachine = async (jobId, jobNumber, machineId, requestingUserId) => {
    const [jobRows] = await db_1.default.query(`SELECT status, machine_id, scheduled_date FROM job_ticket WHERE job_id = ? LIMIT 1`, [jobId]);
    ...
    try {
       ...
       VALUES (?, ?, ?, ?, NOW())`, [jobId, requestingUserId, machineId, today]);
    ...
    await (0, notification_service_1.createNotificationForRoles)(['Admin', 'Manager'], `Job ${jobNumber}: operator started the job`, 'ASSIGNMENT');
    return { success: true, message: 'Job started' };
};
exports.startPrintingOnMachine = startPrintingOnMachine;
// ── Frees the machine when the job leaves the PRINTING stage ─
const finishPrintingForJob = async (jobId) => {
    await db_1.default.query(`UPDATE job_assignment SET finished_at = NOW() WHERE job_id = ? AND finished_at IS NULL`, [jobId]);
};
exports.finishPrintingForJob = finishPrintingForJob;
